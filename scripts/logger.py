...
from scripts.config import Singleton
...
class Logger(metaclass=Singleton):
    def __init__(self):
        ...
        console_formatter = FactsCheckerGptFormatter('%(title_color)s %(message)s')
        ...
        info_formatter = FactsCheckerGptFormatter('%(asctime)s %(levelname)s %(title)s %(message_no_color)s')
        self.file_handler.setFormatter(info_formatter)

        # Error handler error.log
        error_handler = logging.FileHandler(os.path.join(log_dir, error_file))
        error_handler.setLevel(logging.ERROR)
        error_formatter = FactsCheckerGptFormatter(
...
class FactsCheckerGptFormatter(logging.Formatter):
    """
